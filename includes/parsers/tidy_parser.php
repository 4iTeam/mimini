 ...
 * @subpackage Browser
 */
class MiminiTidyPageBuilder implements MiminiParserInterface
{
    private $page;
    private $forms = array();
    ...
    {
        $this->page = new MiminiPage($response,$browser);
    ...
    {
        return $this->insertEmptyTagGuards($this->insertTextareaMiminiWhitespaceGuards($html));
    }
     ...
     *    inserted by Mimini to the tidy output used to make the
     ...
     * @return string     The html with guards inserted.
     */
    private function insertTextareaMiminiWhitespaceGuards($html)
    {
    ...
    /**
     *  Callback for insertTextareaMiminiWhitespaceGuards().
     ...
     * @param object $node Tidy XML node.
     * @return MiminiForm      Facade for MiminiBrowser.
     */
    private function createEmptyForm($node)
    {
        return new MiminiForm($this->tags()->createTag($node->name, (array)$node->attribute), $this->page);
    }
    ...
    private function walkForm($node, $form, $enclosing_label = '')
    {
        if ($node->name == 'a') {
            ...
            if ($node->hasChildren()) {
                foreach ($node->child as $child) {
                    $this->walkForm($child, $form, MiminiPage::normalise($this->innerHtml($node)));
                }
            }
     ...
     * @param MiminiForm $form Form to add it to.
     ...
     * @param MiminiTag $widget Parsed widget to cache.
     */
     ...
     * @param object $node Tidy XML node.
     * @return array            List of MiminiTag options.
     */
     ...
     * @param object $node Tidy XML node.
     * @return array           List of MiminiTag frame descriptions.
     */
     ...
     * @return MiminiTagBuilder    Factory.
     */
    private function tags()
    {
        return new MiminiTagBuilder();
    }

    /**
     *  Called at the end of a parse run. Attaches any
     *  non-wrapping labels to their form elements.
     * @param array $widgets_by_id Cached MiminiTag hash.
     * @param array $labels MiminiTag label elements.
     */
