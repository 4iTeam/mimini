...
class MiminiCompatibility
{
    ...
    {
        if (version_compare(phpversion(), '5') >= 0) {
            return MiminiCompatibility::isIdenticalType($first, $second);
        }
        if ($first != $second) {
            return false;
        }
    ...
    protected static function isIdenticalType($first, $second)
    {
        ...
        if (is_object($first) && is_object($second)) {
            ...
            return MiminiCompatibility::isArrayOfIdenticalTypes(
                (array)$first,
                (array)$second);
        }
        if (is_array($first) && is_array($second)) {
            return MiminiCompatibility::isArrayOfIdenticalTypes($first, $second);
        }
        if ($first !== $second) {
            return false;
        }
    ...
    protected static function isArrayOfIdenticalTypes($first, $second)
    {
        ...
        foreach (array_keys($first) as $key) {
            $is_identical = MiminiCompatibility::isIdenticalType(
                $first[$key],
                $second[$key]);
            if (!$is_identical) {
                return false;
